...
void setTextureMapping(int option)
{
	...
	switch (option)
	{
		case 1:
			...
			break;
		case 2:
			/*img = readImageData("texture/magazine_texture.bmp", &imgWidth, &imgHeight, &channels);
			glGenTextures(texNum, &revolver_magazine_texture);
			glBindTexture(GL_TEXTURE_2D, revolver_magazine_texture);*/
			break;
		case 3:
			glGenTextures(texNum, &R_texture);
			img = readImageData("texture/R2.bmp", &imgWidth, &imgHeight, &channels);
